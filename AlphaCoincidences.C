void AlphaCoincidences()
{
  ...
  TH1F *hV = new TH1F("hV","Drift Velocity",40,-2.0,2.0);
  TH2F *hdtdr = new TH2F("hdtdr","dt vs dR",50,0,200,100,0,1000);
  TH1F *hdZ = new TH1F("hdZ","dZ",50,0,200);

  TH1F *hCSC = new TH1F("hCSC","Scintillation spectrum",80,0,40000);
  TH1F *hCSC1 = new TH1F("hCSC1","Scintillation spectrum first alpha",80,0,40000);
  TH1F *hCSC2 = new TH1F("hCSC2","Scintillation spectrum second alpha",80,0,40000);
  ...
  TH1F *h214Po_fid = new TH1F("h214Po_fid","214Po spectrum (fiducial)",80,0,40000);
  ...
  for (int i = 0; i < nentries-1; i++) {
     for (int k = i+1; k < nentries; k++) {
        ...
        std::cout << "(" << nCoincidences << ") csc1 = " << AlphaCSC1 << "\tcsc2 = " << AlphaCSC2 << "\tdR = " << dR << "\tdt = " << dtAlpha << "\tdZ = " << AlphaZ1 - AlphaZ2 << "\tdt = " << dtAlpha/1000000.0 << "\tfnr1 = " << nr1 << "\tfnr2 = " << nr2 << "\tfne1 = " << ne1 << "\tfne2 = " << ne2 << std::endl;
        ...
        if (TMath::Sqrt(AlphaX1*AlphaX1 + AlphaY1*AlphaY1) < 163 && ((AlphaZ1 > -172 && AlphaZ1 < -20) || (AlphaZ1 > 20 && AlphaZ1 < 172)) && TMath::Sqrt(AlphaX2*AlphaX2 + AlphaY2*AlphaY2) < 163 && ((AlphaZ2 > -172 && AlphaZ2 < -20) || (AlphaZ2 > 20 && AlphaZ2 < 172))) {
           ...
           if (dR == 0.0) {
              hV->Fill((AlphaZ1 - AlphaZ2) /dtAlpha * 1000000.0);
              hdtdr->Fill(AlphaZ1 - AlphaZ2, dtAlpha/1000000.0);
